 ...
 */
	function connect() {
		global $APP_TIMER;
		if( $APP_TIMER && $APP_TIMER->isActive() ) $APP_TIMER->start('MySQL');
		$config = $this->config;
		...
		if( $APP_TIMER && $APP_TIMER->isActive() ) $APP_TIMER->stop('MySQL', 'connect');
 ...
 */
	function disconnect() {
		...
		if( $APP_TIMER && $APP_TIMER->isActive() ) $APP_TIMER->stop('MySQL', 'disconnect');
		return !$this->connected;
	}
 ...
 */
	function _execute($sql) {
		...
		if (preg_match('/^\s*call/i', $sql)) {
			$result = $this->_executeProcedure($sql);
			if( $APP_TIMER && $APP_TIMER->isActive() ) $APP_TIMER->stop('MySQL', $sql);
			return $result;
		}
 ...
 */
	function listSources() {
		...
		global $APP_TIMER;
		if( $APP_TIMER && $APP_TIMER->isActive() ) $APP_TIMER->start('MySQL');
		$sql = 'SHOW TABLES FROM ' . $this->name($this->config['database']) . ';';
