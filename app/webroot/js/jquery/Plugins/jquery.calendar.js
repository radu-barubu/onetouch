...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        //template for month and date xxxxx
        var __SCOLLEVENTTEMP = "<DIV style=\"WIDTH:${width};top:${top};left:${left};BORDER:1px solid WHITE;BORDER-RADIUS:5px\" title=\"${title}\" class=\"event_item_data_details chip chip${i} ${drag}\"><div class=\"dhdV\" style=\"display:none\">${data}</div><DL style=\"BACKGROUND-COLOR:${bgcolor1}; FONT-WEIGHT:bold; PADDING:0 0 5px 0; HEIGHT: ${height}px; \"><DT style=\"BACKGROUND-COLOR:${bgcolor2}; padding: 2px; border-bottom: 1px solid #000000;\">${event_item_title}</DT>${event_item_body}<DD><SPAN></SPAN></DD>";
		//__SCOLLEVENTTEMP += "<DIV class='resizer' style='display:${redisplay}'><DIV class=rszr_icon>&nbsp;</DIV></DIV>";
		__SCOLLEVENTTEMP += "</DL></DIV>";
        ...
        function BuildDaysAndWeekView(startday, l, events, config) {
           // console.log("here");
			...
			/* xxxxx */
            html.push("<div id=\"dvtec\" class=\"scolltimeevent\"><table style=\"position: absolute; top: 0px; left: 0px; margin-top: "+display_position+"px; table-layout: fixed;", jQuery.browser.msie ? "" : "width:100%", "\" cellspacing=\"0\" cellpadding=\"0\"><tbody><tr><td>");
            html.push("<table style=\"height: "+scheduler_height+"px\" id=\"tgTable\" class=\"tg-timedevents\" cellspacing=\"0\" cellpadding=\"0\"><tbody>");
        ...
        function BuildWT(ht, dayarrs, events, dMax) {
            ...
            for (var i = 0; i < dayarrs.length; i++) {
				...
				var dd_str = dayarrs[i].date.format('ddd m/d');
				
				if(current_view == 'day')
				{
					dd_str = dayarrs[i].date.format('ddd');
				}
				
                ht.push("<th abbr='", dateFormat.call(dayarrs[i].date, i18n.xgcalendar.dateformat.fulldayvalue), "' class='gcweekname' scope=\"col\"><div title='", title, "' ", ev, " class='wk-dayname'><span class='", cl, "'>", dd_str, "</span></div></th>");
		...
		{
			/* Doesn't work on Chrome - code moved to line 796-805*/
			/*
            //1:
            ht.push("<tr>");
            ht.push("<td style='width:59px;'></td>");
            ht.push("<td");
            if (dayarrs.length > 1) {
                ht.push(" colSpan='", dayarrs.length, "'");
            }
            ht.push("><div id=\"tgspanningwrapper\" class=\"tg-spanningwrapper\"><div style=\"font-size: 20px\" class=\"tg-hourmarkers\">");
            for (var i = scheduler_begin_hour; i < scheduler_end_hour; i++) {
                ht.push("<div class=\"tg-dualmarker\"></div>");
				
				// Diego Valobra oct 2011 --> HERE I PUSH 1 MORE DIV FOR THE RIGHT SIDE OF THE GRID 
				//ht.push("<div class=\"tg-dualmarker\"></div>");
            }
			...
			*/
            ...
            //2:
            ht.push("<tr>");
            ht.push("<td style=\"width: 59px\" class=\"tg-times\">");

            //get current time 
            var now = new Date(); 
			var h = now.getHours(); 
			var m = now.getMinutes();
            var mHg = gP(h, m) - 4  - (scheduler_begin_hour * hour_item_height); //make middle alignment vertically
           
		   	if(h >= scheduler_begin_hour && h < scheduler_end_hour)
			{
		    	//ht.push("<div class=\"tg-nowptr\" style=\"left:0px;top:", mHg, "px\"></div>");
			}
            
			
			// Diego Valobra oct 2011 --> HERE I OPEN 1 MORE VAR
            ...
            for (var i = scheduler_begin_hour; i < scheduler_end_hour; i++) {
				...
				// Diego Valobra oct 2011 --> HERE I CALL THE 2 FUNCTIONS AND PUSH THE DIVS WE NEED
				tm_ht = fomartTimeShow_half(i);
                ht.push("<div style=\"height: 119px\" class=\"tg-time\">", tmt, "</div>");
				//ht.push("<div style=\"height: 50px\" class=\"tg-time\">", tm_ht, "</div>");
				// Diego Valobra oct 2011 --> AND IT'S DONE
            }
            ht.push("</td>");
			...
			for (var i = 0; i < l; i++) 
			{
				...
				var hourmarkers_html = '';
				hourmarkers_html += "<div id=\"tgspanningwrapper\" class=\"tg-spanningwrapper\"><div style=\"font-size: 20px\" class=\"tg-hourmarkers\">";
				
				for (var a = scheduler_begin_hour; a < scheduler_end_hour; a++) {
					hourmarkers_html += "<div class=\"tg-dualmarker\"></div>";
				}
				
				hourmarkers_html += "</div></div>";
				
				ht.push(hourmarkers_html);
                
				var istoday = dateFormat.call(dayarrs[i].date, "yyyyMMdd") == dateFormat.call(today_date, "yyyyMMdd");
                
				// Today
                if (istoday) 
				{
                    ht.push("<div style=\"margin-bottom: -"+scheduler_height+"px; height:"+scheduler_height+"px\" class=\"tg-today\">&nbsp;</div>");
                }
				
				var is_non_working_days = true;
				
				for(var a in operational_days)
				{
					if(parseInt(operational_days[a]) == parseInt(dayarrs[i].date.getDay()))
					{
						is_non_working_days = false;
					}
				}
				
				if (is_non_working_days) 
				{
                    ht.push("<div style=\"margin-bottom: -"+scheduler_height+"px; height:"+scheduler_height+"px\" class=\"tg-nonworking\">&nbsp;</div>");
                }
				
                //var eventC = $(eventWrap);
                //onclick=\"javascript:FunProxy('rowhandler',event,this);\"
                ht.push("<div  style=\"margin-bottom: -"+scheduler_height+"px; height: "+scheduler_height+"px\" id='tgCol", i, "' class=\"tg-col-eventwrapper\">");
        ...
        function BuildDayEvent(theme, e, index) {
            var approved = e.event[19];
			if(approved=='no')
			{
				var p = { bdcolor: theme[2], bgcolor2: '#000000', bgcolor1: '#000000', width: "70%", icon: "", title: "", data: "" };
			}
			else
			{
				provider_theme = tc(e.event[20]);
                var p = { bdcolor: theme[2], bgcolor2: theme[0], bgcolor1: provider_theme[0], width: "70%", icon: "", title: "", data: "" };
			}

			var start_ampm = 'AM';
			var end_empm = 'AM';
			var start_hour_disp = e.st.hour;
			var end_hour_disp = e.et.hour;
			if(time_format==12){
				if(start_hour_disp >= 12)
				{
					start_ampm = 'PM';
				...
				p.starttime = pZero(start_hour_disp) + ":" + pZero(e.st.minute) + ' ' + start_ampm;
				p.endtime = pZero(end_hour_disp) + ":" + pZero(e.et.minute) + ' ' + end_empm;
			}
			else{
				p.starttime = pZero(start_hour_disp) + "" + pZero(e.st.minute) + ' ' ;
				p.endtime = pZero(end_hour_disp) + "" + pZero(e.et.minute) + ' ' ;				
			}
			p.patient_name = e.event[12];
            p.content = e.event[1];
            p.title = getTitle(e.event);
/*            alert(e.event[1] + '-' + e.event[2] + '-' + e.event[3] + '-' + e.event[4] + '-' + e.event[5] + '-' + e.event[6] + '-' + e.event[7] + '-' + e.event[8] + '-' + e.event[9] + '-' + e.event[10] + '-' + e.event[11] + '-' + e.event[12] + '-' + e.event[13] + '-' + e.event[14] + '-' + e.event[15] + '-' + e.event[16] + '-' + e.event[17] + '-' + e.event[18] + '-' + e.event[19]);*/
			//build title here
			var start_date = e.event[2];
			var end_date = e.event[3];
			p.title = "Location: " + e.event[13] + "\r\n";
			p.title += "Date/Time: " + start_date.format("mmm dd (ddd)") + ' ' + start_date.format('hh:MM TT') + ' - ' + end_date.format('hh:MM TT') + "\r\n";
			p.title += "Patient: " + e.event[12] + "\r\n";
			p.title += "Reason for Visit: " + e.event[18] + "\r\n";
			p.title += "Provider: " + e.event[14] + "\r\n";
			p.title += "Type: " + e.event[15] + "\r\n";
			p.title += "Room: " + e.event[16] + "\r\n";
			p.title += "Status: " + e.event[17] + "\r\n";

			p.data = e.event.join("$");
			
			p.event_item_title = p.starttime + ' &mdash; ' + p.endtime;
			p.event_item_body = p.patient_name + ' ('+e.event[18]+')';
			p.right_margin = '15';
			
			var width_percentage = 90;
			
			if(current_view == 'day')
			{
				p.event_item_title += ' [' + e.event[14] + ']';
				width_percentage = 80;
			}
            ...
            p.icon = icons.join("");
            var sP = gP(e.st.hour, e.st.minute) - adjacent_position;
            var eP = gP(e.et.hour, e.et.minute) - adjacent_position;
            p.top = sP + "px";
            
			//console.log("Left: " + e.left + ", Width: " + e.aQ);
			p.left = (e.left * 100) + "%";
            p.width = (e.aQ * width_percentage) + "%";
        ...
        function BuilderMonthBody(htb, showday, startday, events, bodyHeight) {
            ...
            option.vend = enddate;
            option.datestrshow = showday.format("mmmm yyyy"); //CalDateShow(startdate, enddate);
                ...
                for (var h = 0; h < l; ) {
                    ...
                    if (e) { 
                        ...
                        if (!bs && j == (sc - 1) && z[h] < y[h]) {
                            el++;
                            $.extend(tempdata, { "axis": h, ch: "more", "abbr": dateFormat.call(cday[h], i18n.xgcalendar.dateformat.fulldayvalue), html: (y[h] - z[h]) + i18n.xgcalendar.item + ' ' + 'more', click: "javascript:alert('more event');" });
        ...
        function BuildMonthDayEvent(e, cday, length) {
			...
			//For Appointment Requests by Patient Users
			var approved = e.event[19];
			if(approved=='no')
			{
				var p = { color: '#000000', title: "", extendClass: "", extendHTML: "", data: "" };
			}
			else
			{
                var p = { color: theme[0], title: "", extendClass: "", extendHTML: "", data: "" };
			}            
            ...
            if (!e.allday && !sf) {
				//console.log(e.event);
				var st_dt = e.event[2];
				cen = st_dt.format("hh:MM TT") + " &mdash; " + e.event[12];
                //cen = pZero(e.st.hour) + ":" + pZero(e.st.minute) + " " + e.event[12];
            }
            else {
				/*xxxxx*/
                cen = e.event[12];
        ...
        function gP(h, m) {
            return (h * hour_item_height + parseInt(m / 60 * hour_item_height));
        }
        function gW(ts1, ts2) {
            var t1 = ts1 / hour_item_height;
            var t2 = parseInt(t1);
            var t3 = t1 - t2 >= 0.5 ? 30 : 0;
            var t4 = ts2 / hour_item_height;
            var t5 = parseInt(t4);
        ...
        function gH(y1, y2, pt) {
            ...
            var t1 = (sy1 - pt) / hour_item_height;
            var t2 = parseInt(t1);
            var t3 = t1 - t2 >= 0.5 ? 30 : 0;
            var t4 = (sy2 - pt) / hour_item_height;
            var t5 = parseInt(t4);
            var t6 = t4 - t5 >= 0.5 ? 30 : 0;
        ...
        function tc(d) {
            function zc(c, i) {
                var d = "";
		...
		// Diego Valobra oct 2011 --> HERE ARE THE 2 FUNCTION I ADDED TO CREATE THE 15 and 30 minutes HTML OUTPUT
		
        function fomartTimeShow_half(x) {
			if(time_format==12){
				if(x<12){
					return x < 10 ? "0" + x + ":30 AM" : x + ":30 AM";
				}
				else{
					x=x>12?(x-12):x;
					return x < 10 ? "0" + x + ":30 PM" : x + ":30 PM";
				}
			}
			else{
				if(x<12){
					return x < 10 ? "0" + x + ":30 " : x + ":30 ";
				}
				else{
					return x + ":30 ";
				}				
        ...
        function CalDateShow(startday, endday, isshowtime, isshowweek) {
			if (!endday)
			{
                return startday.format(global_date_format);
            } 
			else
			{
                var strstart = startday.format("mmm d, yyyy");
				var strend = endday.format("mmm d, yyyy");
				
				if(startday.format("m") == endday.format("m"))
				{
					//strend = endday.format("d, yyyy");
					
					return startday.format("mmm d") + ' - ' + endday.format("d") + ', ' + startday.format("yyyy");
				}
				else
				{
					var join = (strend!=""? " — ":"");
        ...
        function buildtempdayevent(sh, sm, eh, em, h, title, w, resize, thindex) {
			...
			var start_ampm = 'AM';
			var end_empm = 'AM';
			var start_hour_disp = sh;
			var end_hour_disp = eh;
            ...
            starttime = pZero(start_hour_disp) + ":" + pZero(sm) + ' ' + start_ampm;
            endtime = pZero(end_hour_disp) + ":" + pZero(em) + ' ' + end_empm;

			event_item_title = starttime + ' &mdash; ' + title;
			...
			var newtemp = Tp(__SCOLLEVENTTEMP, {
                ...
                starttime: starttime,
                endtime: endtime,
				event_item_title: event_item_title,
        ...
        function moreshow(mv) {
            ...
            ctitle.html(dateFormat.call(day, i18n.xgcalendar.dateformat.Md3));
            ccontent.empty();
            //var c = tc()[2];
            ...
            if (left + width >= maxleft) {
                left = offsetMe.left - (me.outerWidth() + 2) * 0.5;
            }
            if (top + height >= maxtop) {
                top = maxtop - height +50;
            }
            var newOff = { left: left-20, top: top+5, "z-index": 180, width: width, "visibility": "visible" };
            cc.css(newOff);
        ...
        function ResizeView() {
			...
			var dvH = $("#calhead").height() + 10;
			option.height = _MH - dvH;
                    ...
                    case 1:
                        ...
                        if (diffy > 11 || diffy < -11 || d.cpwrap) {
                            ...
                            if (!d.cpwrap) {
                                ...
                                var cpwrap = $("<div class='ca-evpi drag-chip-wrapper' style='top:" + (ny-adjacent_position) + "px'/>").html(tempdata);
                            ...
                            if (diffx > 5 || diffx < -5 || diffy > 5 || diffy < -5 || d.cpwrap) {
                                ...
                                if (!d.cpwrap) {
                                    ...
                                    if (d.ny != ny) {
                                        ...
                                        tempdata = buildtempdayevent(gh.sh, gh.sm, gh.eh, gh.em, gh.h, data[1], false, false, data[7]);
                                        d.cpwrap.css("top", (ny-adjacent_position) + "px").html(tempdata);
                                    }
                                    d.ny = ny;
                                }
                            }
            ...
            if (_dragdata) {
                ...
                switch (d.type) {
                    case 1: //day view
                        ...
                        if (!d.cpwrap) {
                            var gh = gH(d.sy, d.sy + hour_item_height, tp);
                            var ny = gP(gh.sh, gh.sm);
