...
(function ($) {
	...
	var winheight= window.innerHeight ? window.innerHeight + 50 :$(window).height(); 
	...
	$.documentCenter = function (el) {
		...
		el.css({
			...
			top: parseInt($(document).scrollTop())+( winheight -$('.bbit-window').height())/2
		})
	};
	...
	$.ShowIfrmDailog = function (url, options) {
		...
		options.caption = "Manage Appointment";
		...
		if (isTouchEnabled()) {
			headtemplete = "<div id='dailog_head_${newid}' class='bbit-window-tl'><div class='bbit-window-tr'><div class='bbit-window-tc'><div style='mozuserselect: none; khtmluserselect: none' class='bbit-window-header' unselectable='on'><div class='bbit-tool bbit-tool-close bbit-tool-touch-close'>Close</div><span class='bbit-window-header-text bbit-window-header-text-touch'>${caption}</span></div></div></div></div>";
		}
		...
		var overlayer = $('<div class="ifr_overlay"></div>').css({
			position: 'absolute',
			left: 0,
			top: 0,
			width: Math.max(document.documentElement.clientWidth, document.body.scrollWidth),
			height: Math.max($(document).height(),$(document).height()),
			zIndex: '998',
			background: '#fff',
			opacity: '0.5'
			...
			return false
                ...
                if (!isTouchEnabled()) {
                    $(window).scroll(function(){
                        $('.bbit-window').css({
                                top: parseInt($(document).scrollTop())+($(window).height()-$('.bbit-window').height())/2
                        });
                    });
                }
                
		$('.ifr_overlay').click(closedialog);
		var isdrag = false;
