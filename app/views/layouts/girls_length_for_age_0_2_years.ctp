...
?>
    ...
    var titleText = 'Stature-for-age Chart for Girls';
    var csvFilePath = '<?php echo $this->Html->url(array('controller' => $current_controller, 'action' => 'vitals', 'task' => 'growthpoints', 'name' => 'length_age', 'encounter_id' => $encounter_id)); ?>';
    $(document).ready(function() {
        ...
        $.post(csvFilePath, function(data)
        {
            ...
            var dot = { data: [[<?php echo $age; ?>, <?php echo $length; ?>]],marker:{ symbol: "circle", radius: 5},showInLegend:false};
            dot.type = 'scatter';
            dot.name = 'Length';
            options.series.push(dot);
               
            var dot = { data: <?php echo json_encode($previous_values['stature_age_in_month']); ?>, marker:{ symbol: "circle", radius: 3}, showInLegend:false};
            dot.type = 'scatter';
            dot.lineWidth = 1;
            dot.name = 'Weight';
            options.series.push(dot);
