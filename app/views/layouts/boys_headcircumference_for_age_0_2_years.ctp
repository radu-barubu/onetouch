...
?>
    ...
    var titleText = 'Headcircumference-for-age Chart';
    var csvFilePath = '<?php echo $this->Html->url(array('controller' => $current_controller, 'action' => 'vitals', 'task' => 'growthpoints', 'name' => 'headcircumference_age', 'encounter_id' => $encounter_id)); ?>';
    $(document).ready(function() {
        ...
        $.post(csvFilePath, function(data)
        {
            ...
            var dot = { data: [[<?php echo $age; ?>, <?php echo $headcircumference; ?>]], marker:{ symbol: "circle", radius: 5}, showInLegend:false};
            dot.type = 'scatter';
            dot.name = 'Headcircumference';
            options.series.push(dot);
            
            var dot = { data: <?php echo json_encode($previous_values['head_age_in_month']); ?>, marker:{ symbol: "circle", radius: 3}, showInLegend:false};
            dot.type = 'scatter';
            dot.lineWidth = 1;
            dot.name = 'Weight';
            options.series.push(dot);
            
            options.series.chart = new Highcharts.Chart(options);
        });			
    });
</script>		
