...
Derived from HL7 Finland R2 Tyylitiedosto: Tyyli_R2_B3_01.xslt Updated by   Calvin E. Beebe,   Mayo Clinic - Rochester Mn.
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-bottom: 20px;">
            ...
            <tr>
                <td style="padding-top: 10px;"><span class="btn" onClick="parent.importPatient('<?php echo $folder; ?>', 'error_msg1', '<?php echo $validate_mode; ?>', '<?php echo $patient_id; ?>');">Import</span> <span class="btn" onClick="parent.closeImportIframe();">Cancel</span></td>
            </tr>
        </table>
        <?php endif; ?>
        <h1><span style="font-size: <?php echo ($display_settings['body_font_size'] + 2); ?>px;">Continuity of Care Document</span></h1>
        <br/>
        
        <xsl:variable name="patientRole" select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole"/>
        <table width="75%" cellspacing="3" cellpadding="1" bgcolor="#ffffcc" id="ccrheader">
            <tr>
            ...
            <tr>
                ...
                <td><xsl:value-of select="$patientRole/n1:id/@extension"/></td>
            </tr>
            <tr>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-bottom: 20px; margin-top: 20px;">
            ...
            <tr>
                <td style="padding-top: 10px;"><span class="btn" onClick="parent.importPatient('<?php echo $folder; ?>', 'error_msg2', '<?php echo $validate_mode; ?>', '<?php echo $patient_id; ?>');">Import</span> <span class="btn" onClick="parent.closeImportIframe();">Cancel</span></td>
            </tr>
        </table>
        <?php endif; ?>
        </body>
    ...
    <xsl:template name="getName">
        ...
        <xsl:choose>
            <xsl:when test="$name/n1:family">
                <xsl:value-of select="$name/n1:given"/><xsl:text> </xsl:text><xsl:value-of select="$name/n1:family"/><xsl:text> </xsl:text>
                <xsl:if test="$name/n1:suffix">
                    <xsl:text>, </xsl:text><xsl:value-of select="$name/n1:suffix"/>
                </xsl:if>
            </xsl:when>
            <xsl:otherwise>
...
-->
    ...
    <xsl:template match="n1:th">
        <th><xsl:copy-of select="@*"/>
            <xsl:apply-templates/>
        </th>
    </xsl:template>
    <xsl:template match="n1:td">
        <td><xsl:copy-of select="@*"/>
            <xsl:apply-templates/>
        </td>
    </xsl:template>
...
-->
    ...
    <xsl:template name="bottomline">
        <p><br/><b><xsl:text>Electronically generated by: </xsl:text></b>
            <xsl:call-template name="getName">
                <xsl:with-param name="name" 
