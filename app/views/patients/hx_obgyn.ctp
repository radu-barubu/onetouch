...
$addURL = $html->url(array('action' => 'hx_obgyn', 'patient_id' => $patient_id, 'task' => 'addnew')) . '/';
$mainURL = $html->url(array('action' => 'hx_obgyn', 'patient_id' => $patient_id)) . '/';
...
<script language="javascript" type="text/javascript">
    ...
    {  
		...
		jQuery.validator.addMethod("minDate", function(value, element, params) { 
			var
				givenDate = Admin.getDateObject(element);
			
			
			
			return this.optional(element) || (givenDate.getTime() >= patientDOB.getTime() ); 
		}, "Date cannot be set earlier than patient birthdate");		


		$(".datemask", $("#frmObGynRecords")).each(function(){
			$(this).rules("add", {
				currentDate: true,
				minDate: true
			});
		...
		$('#last_menstrual_period', $("#frmObGynRecords")).rules('add', {
			dateRange: {
				from: '#age_started_period',
				to: '#last_menstrual_period'
			}
		});
...
<div style="overflow: hidden;">	
	<div class="title_area">
		<div class="title_text">
			...
			{
				?><a href="javascript:void(0);" class="active"  url="<?php echo $html->url(array('controller' => 'patients', 'action' => 'hx_obgyn', 'patient_id' => $patient_id)); ?>">Ob/Gyn History</a><?php
			}
			?>
		</div>
    </div>
         ...
         {?>
			...
			<form id="frmObGynRecordsGrid" method="post" action="<?php echo $thisURL; ?>" accept-charset="utf-8" enctype="multipart/form-data">
                <table id="table_medical" cellpadding="0" cellspacing="0"  class="listing"> 
                    ...
                    ?>
                    <tr editlinkajax="<?php echo $html->url(array('action' => 'hx_obgyn', 'task' => 'edit', 'patient_id' => $patient_id, 'ob_gyn_history_id' => $Patientobgyn_record['PatientObGynHistory']['ob_gyn_history_id'])); ?>">
			...
			<div class="paging">
                ...
                <?php 
                    ...
                    {
                        echo $paginator->prev('<< Previous', array('model' => 'PatientObGynHistory', 'url' => $paginator->params['pass']), null, array('class'=>'disabled')); 
                    }
                ?>
                ...
                <?php 
                    ...
                    {
                        echo $paginator->next('Next >>', array('model' => 'PatientObGynHistory', 'url' => $paginator->params['pass']), null, array('class'=>'disabled')); 
                    }
                ?>
            </div>
         <?php } ?>
...
$(function(){
	$('.datemask').datepicker('option', 'maxDate', '+0d');	
