...
$addURL = $html->url(array('action' => 'hx_social', 'patient_id' => $patient_id, 'task' => 'addnew')) . '/';
$mainURL = $html->url(array('action' => 'hx_social', 'patient_id' => $patient_id)) . '/';
...
<div style="overflow: hidden;">    
    <div class="title_area">
		<div class="title_text">
            ...
            <a href="javascript:void(0);" class="active" url="<?php echo $html->url(array('controller' => 'patients', 'action' => 'hx_social', 'patient_id' => $patient_id)); ?>">Social History</a>
         ...
         ?>
             ...
             <?php echo $id_field; ?> 
				...
				if(sizeof($last_user) > 0) { echo '<em>';
                 ...
                 <div class="actions">
                 <ul>
                    <li removeonread="true"><a href="javascript: void(0);" onclick="$('#frmSocialRecords').submit();"><?php echo ($task == 'addnew') ? 'Add' : 'Save'; ?></a></li>
         ...
         {?>
			...
			<form id="frmSocialRecordsGrid" method="post" action="<?php echo $thisURL; ?>" accept-charset="utf-8" enctype="multipart/form-data">
                <table id="table_medical" cellpadding="0" cellspacing="0"  class="listing"> 
                    <tr>                
						...
						<th><?php echo $paginator->sort('Status', 'status', array('model' => 'PatientSocialHistory', 'class' => 'ajax'));?></th>
                     </tr>
                     <?php
                    $i = 0;
                    foreach ($PatientSocialHistory as $PatientSocial_record):
                    ?>
                    <tr editlinkajax="<?php echo $html->url(array('action' => 'hx_social', 'task' => 'edit', 'patient_id' => $patient_id, 'social_history_id' => $PatientSocial_record['PatientSocialHistory']['social_history_id'])); ?>">
			...
			<div class="paging">
                ...
                <?php 
                    ...
                    {
                        echo $paginator->prev('<< Previous', array('model' => 'PatientSocialHistory', 'url' => $paginator->params['pass']), null, array('class'=>'disabled')); 
                    }
                ?>
                ...
                <?php 
                    ...
                    {
                        echo $paginator->next('Next >>', array('model' => 'PatientSocialHistory', 'url' => $paginator->params['pass']), null, array('class'=>'disabled')); 
                    }
                ?>
            </div> 
         <?php } ?>
