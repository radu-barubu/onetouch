...
/*
			if (@count($v['EncounterMaster'][$i]['EncounterLabs']) > 0)
			{
				for ($j = 0; $j < count($v['EncounterMaster'][$i]['EncounterLabs']); ++$j)
				{
					$row = array();
					...
					$row[] = $v['EncounterMaster'][$i]['EncounterLabs'][$j]['lab_test_result'];
...
<form id="frm" method="post" action="<?php echo $this->Session->webroot.'reports/patient_lists/task:export'; ?>" accept-charset="utf-8" enctype="multipart/form-data" target="_blank">
<?php
