 ...
 */
	function next($title = 'Next >>', $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		return $this->__pagingLink('Next', $title, $options, $disabledTitle, $disabledOptions) . $this->specific_page($options['model']);
	}
 ...
 */
	function numbers($options = array()) {
		...
		if($cur_model && $this->hasNext($cur_model)===false)
			$out .= $this->specific_page($cur_model);
 ...
 */
	...
	function specific_page($model)
	{
		...
		if(isset($named['page']))
			unset($named['page']);
		$url_args = array_merge(array('controller' => $this->params['controller'], 'action' => $this->params['action']), $named);
		$url = $this->Html->Url($url_args);		
		$nums = '';
		for($i = 0; $i <= $pageCount; $i++) {
			$nums .= '<a id="'.$model.'_page_'.$i.'" style="display:none;" href="'.$url.'/page:'.$i.'"></a>';
		}
		$return = '<span class="goto_btn smallbtn" onclick="goto(\''.$model.'_page_\', this);">Go To</span><span>'.$nums.'<input class="specific_page_num" type="text" ></span>';
		return $return;
	}
}
