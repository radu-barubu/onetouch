...
$mainURL = $html->url(array('action' => 'in_house_work_meds', 'encounter_id' => $encounter_id)) . '/';
...
<script language="javascript" type="text/javascript">
    function addData(point_of_care_id, drug)
    {
        
        var val = (jQuery('#drug_'+point_of_care_id).is(':checked'))?'1':'0';

        if(val==1)
        {           
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_meds/encounter_id:<?php echo $encounter_id; ?>/task:addMedsTest/', 
            {
              'data[administration_point_of_care_id]': point_of_care_id,
              'data[item_value]': drug
            }, 
            function(data)
            {
            }
            ...
            $('#label_drug_'+point_of_care_id).css('cursor', 'pointer');
        }
        else
        {
            $('#in_house_work_drug_form_area').hide();
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_meds/encounter_id:<?php echo $encounter_id; ?>/task:deleteMedsTest/', 
            {
              'data[item_value]' : drug
            }, 
            function(data)
            ...
            $('#label_drug_'+point_of_care_id).css('cursor', 'auto');
        }
    }
    
    function loadData(point_of_care_id, drug)
    {
       var val = (jQuery('#drug_'+point_of_care_id).is(':checked'))?'1':'0';
        if(val==1)
        {
        $('#in_house_work_drug_form_area').show();
        $('#in_house_work_drug_form_area').html('');
        $("#imgLoad").show();
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_meds/encounter_id:<?php echo $encounter_id; ?>/task:checkMedsTest/', 
            {
              'data[item_value]' : drug
            }, 
            function(data)
            {
               ...
               if(data.drug_test['exist']=='yes')
               {
                  $.post('<?php echo $this->Session->webroot; ?>encounters/in_house_work_meds_data/encounter_id:<?php echo $encounter_id; ?>/',     {'drug':drug}, 
                  function(data)
                  {
                  	$('#in_house_work_drug_form_area').html(data);
                    $("#imgLoad").hide();
	...
	function loadInitialData()
	{
		...
		$.post('<?php echo $this->Session->webroot; ?>encounters/<?php echo $this->params['action']; ?>_data/encounter_id:<?php echo $encounter_id; ?>/', 'drug='+'<?php echo $init_point_of_care_name; ?>', 
		function(data){ 
		   $('#in_house_work_drug_form_area').html(data);
		   $("#imgLoad").hide();
		   if(typeof($ipad)==='object')$ipad.ready();
    ...
    {   
        ...
        $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_meds/encounter_id:<?php echo $encounter_id; ?>/task:get_admin_poc_med/', 
            '', 
            function(data)
            {
            //alert(data.lab_items.length);
            if(data.med_items.length > 0)
                {            
                    for(var i = 0; i < data.med_items.length; i++)
                    {
                    var point_of_care_id = data.med_items[i].AdministrationPointOfCare.point_of_care_id;
        ...
        $('#previousRecordsbtn').click(function()
        {
            ...
            loadTab($(this), "<?php echo $html->url(array('controller' => 'encounters', 'action' => 'poc_previous_records', 'encounter_id' => $encounter_id)); ?>");
				...
				$('.fast-chkbox').each(function(){
					...
					var drug_name = $(this).data('poc_drug_name');
					$(this).click(function(){
						addData(poc_id, drug_name);
				...
				$('.poc-button').each(function(){
					...
					var drug_name = $(this).closest('.poc-wrapper').find('.fast-chkbox').data('poc_drug_name');
					
					
					$(this).click(function(){
						loadData(poc_id, drug_name);
					});
				});         
...
<div id="poc_area" style="overflow: hidden;">
    <div class="title_area">
        <div class="title_text">
            ...
            <a href="javascript:void(0);" class="in_house_work_submenuitem active" url="<?php echo $html->url(array('controller' => 'encounters', 'action' => 'in_house_work_meds', 'encounter_id' => $encounter_id)); ?>" style="float: none;">Meds</a>
    ...
    <div id="lab_records_area" class="tab_area"> 
        <div> 
            ...
            $drug = $AdministrationPointOfCare['AdministrationPointOfCare']['drug'];
            ?>
            <div class="poc-wrapper" ><label class="label_check_box_home" id="label_drug_<?php echo $point_of_care_id; ?>" name="label_drug_<?php echo $point_of_care_id; ?>" ><input name="drug_<?php echo $point_of_care_id; ?>" id="drug_<?php echo $point_of_care_id; ?>" type="checkbox" class="fast-chkbox" value="<?php echo $point_of_care_id; ?>" <?php echo (in_array(strtolower($drug), $EncounterPointOfCare))?'checked':''; ?> data-poc_id="<?php echo $point_of_care_id; ?>" data-poc_drug_name="<?php echo $drug; ?>" /> &nbsp;<?php echo $drug; ?></label><button class="poc-button" id="rerun_<?php echo $point_of_care_id; ?>" >&nbsp;</button></div>
            <?php endforeach; ?>
    ...
    <div class="clear"></div>
    <div id="lab_records_area" class="tab_area"> 
        <div id="in_house_work_drug_form_area" style="padding-top:15px;"></div>
    </div>
</div>
<style>
