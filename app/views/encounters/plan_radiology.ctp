...
<script language="javascript" type="text/javascript">
	function addPlan(item_value)
	{
		...
		$.post(
			'<?php echo $this->Session->webroot; ?>encounters/plan_radiology/encounter_id:<?php echo $encounter_id; ?>/task:addPlan/', 
			formobj.serialize(), 
			function(data)
			{
				$("#imgLoading").hide();
				$("#plan_search_radiology").val("");
				resetPlan(data);
			},
			'json'
		);
	...
	function deletePlan(item_value)
	{
		...
		formobj.append('<input name="diagnosis" type="hidden" value="'+diagnosis+'">');
		$.post(
			'<?php echo $this->Session->webroot; ?>encounters/plan_radiology/encounter_id:<?php echo $encounter_id; ?>/task:deletePlan/', 
			formobj.serialize(), 
			function(data)
			{
				$("#imgLoadingDel").hide();
				resetPlan(data);
				$('#radiology_form_area').html('');
			},
			'json'
		);
	}
	...
	{
		$("#table_plan_radiology_list tr").each(function()
		{
		...
		if(data.length > 0)
		{
			for(var i = 0; i < data.length; i++)
			{
				...
				$("#table_plan_radiology_list").append(html);
			}
			
			$("#table_plan_radiology_list tr:even td").addClass("striped");
			
			<?php if($page_access == 'W'): ?>
			$(".del_icon", $("#table_plan_radiology_list")).click(function()
			{
			...
			$("#table_plan_radiology_list tr:even").each(function()
			{
			...
			$("#table_plan_radiology_list tr").not('#table_plan_radiology_list tr:first').each(function()
			{
			    $('td', $(this)).not('td:first', $(this)).each(function()
				{
					...
					{	
						...
						var procedure_name = $(this).parent().attr("itemvalue");					
						var diagnosis = $("#table_plans_table").attr("planname");
						$('#radiology_form_area').html('');
						$("#imgLoadPlanRadiologyForm").show();
						$.post('<?php echo $this->Session->webroot; ?>encounters/plan_radiology_data/encounter_id:<?php echo $encounter_id; ?>/', 
						'diagnosis='+diagnosis+'&procedure_name='+procedure_name, 
						function(data)
						{
							$('#radiology_form_area').html(data);
							$("#imgLoadPlanRadiologyForm").hide();
							$('#radiology_form_area').attr('planname', procedure_name);
		...
		else
		{
			...
			html += '<td colspan="2">No Radiology</td>';
			html += '</tr>';
	...
	function resetPlan(items)
	{
		...
		{
			$.post(
				'<?php echo $this->Session->webroot; ?>encounters/plan_radiology/encounter_id:<?php echo $encounter_id; ?>/task:get_plans/', 
				'diagnosis='+diagnosis,
				function(data)
				{
					resetPlanTable(data);
	...
	$(document).ready(function()
	{
		...
		$("#plan_search_radiology").autocomplete('<?php echo $this->Session->webroot; ?>encounters/plan_radiology/encounter_id:<?php echo $encounter_id; ?>/task:radiology_search/', {
		max: 20,
		mustMatch: false,
		matchContains: false,
                extraParams: {
...
<div>
	...
	<div style="margin-top: 10px; text-align: left;">
		...
		<table class="form" style="margin: 0pt auto;" align="left" cellpadding="0" cellspacing="0">
			<tr>
				<td class="top_pos"><label>Radiology:</label></td>
				<td><input type='text' name='plan_search_radiology' id='plan_search_radiology' style="width: 350px;">
				<a class="btn search_plan_add_btn" href="javascript:void(0);" style="float: none;" targettext="plan_search_radiology" onclick="addPlan($('#plan_search_radiology').val());">Add</a></td>
	...
	<div style="text-align: left; width: 100%;">
		...
		<div style="float: right; width: 50%;">
			<div style="padding-left: 5px;">
				<table id="table_plan_radiology_list" class="small_table" style="width: 100%;" cellpadding="0" cellspacing="0">
					<tbody>
					<tr deleteable="false">
						<th colspan="2">Ordered</th>
					</tr>					
				    <tr deleteable="true">
					    <td colspan="2">No Radiology</td>
					</tr>
					</tbody>
				</table>
			</div>
	...
	<div style="clear: both;">&nbsp;</div>
	 <span id="imgLoadPlanRadiologyForm" style="float: center; display:none; margin-top: -2px;"><?php echo $html->image('ajax_loaderback.gif', array('alt' => 'Loading...')); ?></span>
<div style="text-align: left; width: 100%; margin-top: 10px; float:left" id="radiology_form_area">
</div>
</div>
