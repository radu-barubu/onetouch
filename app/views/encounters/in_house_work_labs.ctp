...
<script language="javascript" type="text/javascript">
    function addData(point_of_care_id, lab_test_name)
    {
        var val = (jQuery('#lab_test_'+point_of_care_id).is(':checked'))?'1':'0';

        if(val==1)
        {           
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_labs/encounter_id:<?php echo $encounter_id; ?>/task:addLabTest/', 
            {
              'data[administration_point_of_care_id]': point_of_care_id,
              'data[item_value]': lab_test_name
            }, 
            function(data)
            {
            });
            $('#label_lab_test_'+point_of_care_id).css('cursor', 'pointer');
        }
        else
        {
            $('#in_house_work_lab_form_area').hide();
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_labs/encounter_id:<?php echo $encounter_id; ?>/task:deleteLabTest/', 
            {
              'data[item_value]' : lab_test_name
            }, 
            function(data)
            ...
            $('#label_lab_test_'+point_of_care_id).css('cursor', 'auto');
        }
    }
    
    function loadData(point_of_care_id, lab_test_name)
    {
        var val = (jQuery('#lab_test_'+point_of_care_id).is(':checked'))?'1':'0';
        
				if(val==1)
        {
        	$('#in_house_work_lab_form_area').show();
        	$('#in_house_work_lab_form_area').html('');
        	$("#imgLoad").show();
            $.post(
            	'<?php echo $this->Session->webroot; ?>encounters/in_house_work_labs/encounter_id:<?php echo $encounter_id; ?>/task:checkLabTest/', 
                {
                   'data[item_value]' : lab_test_name
                }, 
            	function(data){
								if(data.lab_test['exist']=='yes'){
									$.post('<?php echo $this->Session->webroot; ?>encounters/in_house_work_labs_data/encounter_id:<?php echo $encounter_id; ?>/', {'lab_test_name':lab_test_name}, 
									function(data){ 
										$('#in_house_work_lab_form_area').html(data);
										$("#imgLoad").hide();
	...
	function loadInitialData()
	{
		...
		$.post('<?php echo $this->Session->webroot; ?>encounters/<?php echo $this->params['action']; ?>_data/encounter_id:<?php echo $encounter_id; ?>/', 'lab_test_name='+'<?php echo $init_point_of_care_name; ?>', 
		function(data)
		{ 
		   $('#in_house_work_lab_form_area').html(data);
		   $("#imgLoad").hide();
	...
	$('.label_check_box_home').children("input:checked").map(function() {
        ...
        $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_labs/encounter_id:<?php echo $encounter_id; ?>/task:get_admin_poc_labs/', 
            '', 
            function(data)
            {
                if(data.lab_items.length > 0)
                {            
                    for(var i = 0; i < data.lab_items.length; i++)
                    {
                        var point_of_care_id = data.lab_items[i].AdministrationPointOfCare.point_of_care_id;
           ...
           $("#sub_tab_table").css('display', 'none');
            ...
            loadTab($(this), "<?php echo $html->url(array('controller' => 'encounters', 'action' => 'poc_previous_records', 'encounter_id' => $encounter_id)); ?>");
				...
				$('.fast-chkbox').each(function(){
					...
					var test_name = $(this).data('poc_test_name');
					$(this).click(function(){
						addData(poc_id, test_name);
				...
				$('.poc-button').each(function(){
					...
					var test_name = $(this).closest('.poc-wrapper').find('.fast-chkbox').data('poc_test_name');
					
					
					$(this).click(function(){
						loadData(poc_id, test_name);
					});
				});
...
<div id="poc_area" style="overflow: hidden;">
    <div class="title_area">
        <div class="title_text">
            <a href="javascript:void(0);" class="in_house_work_submenuitem active" url="<?php echo $html->url(array('controller' => 'encounters', 'action' => 'in_house_work_labs', 'encounter_id' => $encounter_id)); ?>" style="float: none;">Labs</a>
    ...
    <div id="lab_records_area" class="tab_area"> 
        <div>
            ...
            $lab_test_name = $AdministrationPointOfCare['AdministrationPointOfCare']['lab_test_name'];
            ?>
            <div class="poc-wrapper"><label class="label_check_box_home" id="label_lab_test_<?php echo $point_of_care_id; ?>" name="label_lab_test_<?php echo $point_of_care_id; ?>" ><input name="lab_test_<?php echo $point_of_care_id; ?>" id="lab_test_<?php echo $point_of_care_id; ?>" type="checkbox" class="fast-chkbox" value="<?php echo $point_of_care_id; ?>" <?php echo (in_array(strtolower($lab_test_name), $EncounterPointOfCare))?'checked':''; ?> data-poc_id="<?php echo $point_of_care_id; ?>" data-poc_test_name="<?php echo $lab_test_name; ?>" />&nbsp;<?php echo $lab_test_name; ?></label><button class="poc-button" id="rerun_<?php echo $point_of_care_id; ?>" >&nbsp;</button></div>
            <?php     
            endforeach; ?>
        </div>
    </div>
    ...
    <div class="clear"></div>
    <div id="lab_records_area" class="tab_area"> 
        <div id="in_house_work_lab_form_area" style="padding-top:15px;"></div>
    </div>
</div>
<style>
