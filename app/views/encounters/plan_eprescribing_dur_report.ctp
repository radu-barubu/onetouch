...
<?php
    ...
    $second_iframe_src = 'https://'.$host.'/servlet/DxLogin?userid='.urlencode($user_id).'&PW='.urlencode($password).'&hdnBusiness='.urlencode($facility).'&target=jsp/rx/rx/DURhistory.jsp&actionCommand=durSummary&P_ACT='.urlencode($mrn).'&P_FNM='.urlencode($patient_fname).'&P_LNM='.urlencode($patient_lname).'&P_DOB='.urlencode(__date("m/d/Y", strtotime($patient_dob))).'&apiLogin=true&textError=true';
?>
<script language="javascript" type="text/javascript">
...
<div id="div_dur_report" style="width: 100%; padding-top:50px; padding-bottom:20px;">
<form name="form_dur_report" id="form_dur_report" method="post" >
