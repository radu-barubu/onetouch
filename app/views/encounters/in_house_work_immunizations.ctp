...
$mainURL = $html->url(array('action' => 'in_house_work_immunization', 'encounter_id' => $encounter_id)) . '/';
...
<script language="javascript" type="text/javascript">
    ...
    function addData(point_of_care_id, vaccine_name, vaccine_reason, vaccine_priority)
    {
        
        var val = (jQuery('#vaccine_'+point_of_care_id).is(':checked'))?'1':'0';
        if(val==1)
        {           
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_immunizations/encounter_id:<?php echo $encounter_id; ?>/task:addImmunizationTest/', 
            {
              'data[administration_point_of_care_id]' : point_of_care_id,
              'data[item_value]': vaccine_name,
              'data[item_value_reason]': vaccine_reason,
              'data[item_value_priority]': vaccine_priority
            }, 
            function(data)
            {
            }
            );
            $('#label_immunization_'+point_of_care_id).css('cursor', 'pointer');
        }
        else
        {
            $('#in_house_work_immunization_form_area').hide();
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_immunizations/encounter_id:<?php echo $encounter_id; ?>/task:deleteImmunizationTest/', 
              {
                 'data[item_value]': vaccine_name
              }, 
            function(data)
            ...
            $('#label_immunization_'+point_of_care_id).css('cursor', 'auto');
        }
    }
    
    function loadData(point_of_care_id, vaccine_name)
    {
        var val = (jQuery('#vaccine_'+point_of_care_id).is(':checked'))?'1':'0';
        if(val==1)
        {
        $('#in_house_work_immunization_form_area').show();
        $('#in_house_work_immunization_form_area').html('');
        $("#imgLoad").show();
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_immunizations/encounter_id:<?php echo $encounter_id; ?>/task:checkImmunizationTest/', 
              {
                 'data[item_value]': vaccine_name
              }, 
            function(data)
            {
               ...
               if(data.immunization_test['exist']=='yes')
               {
                   $.post('<?php echo $this->Session->webroot; ?>encounters/in_house_work_immunizations_data/encounter_id:<?php echo $encounter_id; ?>/',     {'vaccine_name':vaccine_name}, 
                   function(data)
                   {
                       $('#in_house_work_immunization_form_area').html(data);
                       $("#imgLoad").hide();
	...
	function loadInitialData()
	{
		...
		$.post('<?php echo $this->Session->webroot; ?>encounters/<?php echo $this->params['action']; ?>_data/encounter_id:<?php echo $encounter_id; ?>/', 'vaccine_name='+'<?php echo $init_point_of_care_name; ?>', 
		function(data)
		{ 
		   $('#in_house_work_immunization_form_area').html(data);
		   $("#imgLoad").hide();
    ...
    {   
        ...
        $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_immunizations/encounter_id:<?php echo $encounter_id; ?>/task:get_admin_poc_immunization/', 
            '', 
            function(data)
            {
            //alert(data.lab_items.length);
            if(data.immunization_items.length > 0)
                {            
                    for(var i = 0; i < data.immunization_items.length; i++)
                    {
                    var point_of_care_id = data.immunization_items[i].AdministrationPointOfCare.point_of_care_id;
        ...
        $('#previousRecordsbtn').click(function()
        {
            ...
            loadTab($(this), "<?php echo $html->url(array('controller' => 'encounters', 'action' => 'poc_previous_records', 'encounter_id' => $encounter_id)); ?>");
				...
				$('.fast-chkbox').each(function(){
					...
					var vaccine_name = $(this).data('poc_vaccine_name');
					var vaccine_reason = $(this).data('poc_vaccine_reason');
					var vaccine_priority = $(this).data('poc_vaccine_priority');
					$(this).click(function(){
						addData(poc_id, vaccine_name, vaccine_reason, vaccine_priority);
				...
				$('.poc-button').each(function(){
					...
					var vaccine_name = $(this).closest('.poc-wrapper').find('.fast-chkbox').data('poc_vaccine_name');
					
					
					$(this).click(function(evt){
            evt.preventDefault();
            evt.stopPropagation();
						loadData(poc_id, vaccine_name);
					});
				});          
...
<div id="poc_area" style="overflow: hidden;">
    <div class="title_area">
        <div class="title_text">
            ...
            <a href="javascript:void(0);" class="in_house_work_submenuitem active" url="<?php echo $html->url(array('controller' => 'encounters', 'action' => 'in_house_work_immunizations', 'encounter_id' => $encounter_id)); ?>" style="float: none;">Immunization</a>
    ...
    <div id="lab_records_area" class="tab_area"> 
        <div> 
            ...
            $vaccine_name = $AdministrationPointOfCare['AdministrationPointOfCare']['vaccine_name'];
			$vaccine_reason = $AdministrationPointOfCare['AdministrationPointOfCare']['vaccine_reason'];
			$vaccine_priority = $AdministrationPointOfCare['AdministrationPointOfCare']['vaccine_priority'];
            ?>
            <div class="poc-wrapper"><label class="label_check_box_home" id="label_vaccine_<?php echo $point_of_care_id; ?>" name="label_vaccine_<?php echo $point_of_care_id; ?>" ><input name="vaccine_<?php echo $point_of_care_id; ?>" id="vaccine_<?php echo $point_of_care_id; ?>" type="checkbox" class="fast-chkbox" value="<?php echo $point_of_care_id; ?>" <?php echo (in_array(strtolower($vaccine_name), $EncounterPointOfCare))?'checked':''; ?> data-poc_id="<?php echo $point_of_care_id; ?>" data-poc_vaccine_name="<?php echo $vaccine_name; ?>" data-poc_vaccine_reason="<?php echo $vaccine_reason; ?>" data-poc_vaccine_priority="<?php echo $vaccine_priority; ?>" />&nbsp;<?php echo $vaccine_name; ?></label><button class="poc-button" id="rerun_<?php echo $point_of_care_id; ?>" >&nbsp;</button></div>
            <?php endforeach; ?>
    ...
    <div class="clear"></div>
    <div  id="lab_records_area" class="tab_area">
        <div id="in_house_work_immunization_form_area" style="padding-top:15px;"></div>
    </div>
</div>
<style>
