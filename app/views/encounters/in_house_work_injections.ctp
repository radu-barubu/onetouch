...
$mainURL = $html->url(array('action' => 'in_house_work_injections', 'encounter_id' => $encounter_id)) . '/';
...
function addData(point_of_care_id, injection_name)
    {
        
        var val = (jQuery('#injection_'+point_of_care_id).is(':checked'))?'1':'0';

        if(val==1)
        {           
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_injections/encounter_id:<?php echo $encounter_id; ?>/task:addInjectionTest/', 
            {
              'data[administration_point_of_care_id]': point_of_care_id,
              'data[item_value]': injection_name
            }, 
            function(data)
            {
            }
            );
            $('#label_injection_'+point_of_care_id).css('cursor', 'pointer');
        }
        else
        {
            $('#in_house_work_injection_form_area').hide();
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_injections/encounter_id:<?php echo $encounter_id; ?>/task:deleteInjectionTest/', 
            {
              'data[item_value]' : injection_name
            }, 
            function(data)
            ...
            $('#label_injection_'+point_of_care_id).css('cursor', 'auto');
        }
    }
    
    function loadData(point_of_care_id, injection_name)
    {
        var val = (jQuery('#injection_'+point_of_care_id).is(':checked'))?'1':'0';
        if(val==1)
        {    
        $('#in_house_work_injection_form_area').show();
        $('#in_house_work_injection_form_area').html('');
        $("#imgLoad").show();
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_injections/encounter_id:<?php echo $encounter_id; ?>/task:checkInjectionTest/', 
            {
              'data[item_value]' : injection_name
            }, 
            function(data)
            {
               ...
               if(data.injection_test['exist']=='yes')
               {
                  $.post('<?php echo $this->Session->webroot; ?>encounters/in_house_work_injections_data/encounter_id:<?php echo $encounter_id; ?>/',     {'injection_name':injection_name}, 
                  function(data)
                  {
                  	$('#in_house_work_injection_form_area').html(data);
                  	$("#imgLoad").hide();
	...
	function loadInitialData()
	{
		...
		$.post('<?php echo $this->Session->webroot; ?>encounters/<?php echo $this->params['action']; ?>_data/encounter_id:<?php echo $encounter_id; ?>/', 'injection_name='+'<?php echo $init_point_of_care_name; ?>', 
		function(data)
		{ 
		   $('#in_house_work_injection_form_area').html(data);
		   $("#imgLoad").hide();
    ...
    {   
        ...
        $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_injections/encounter_id:<?php echo $encounter_id; ?>/task:get_admin_poc_injection/', 
            '', 
            function(data)
            {
            //alert(data.lab_items.length);
            if(data.injection_items.length > 0)
                {            
                    for(var i = 0; i < data.injection_items.length; i++)
                    {
                    var point_of_care_id = data.injection_items[i].AdministrationPointOfCare.point_of_care_id;
        ...
        $('#previousRecordsbtn').click(function()
        {
            ...
            loadTab($(this), "<?php echo $html->url(array('controller' => 'encounters', 'action' => 'poc_previous_records', 'encounter_id' => $encounter_id)); ?>");
				...
				$('.fast-chkbox').each(function(){
					...
					var injection_name = $(this).data('poc_injection_name');
					$(this).click(function(){
						addData(poc_id, injection_name);
				...
				$('.poc-button').each(function(){
					...
					var injection_name = $(this).closest('.poc-wrapper').find('.fast-chkbox').data('poc_injection_name');
					
					
					$(this).click(function(){
						loadData(poc_id, injection_name);
					});
				});         
...
<div id="poc_area" style="overflow: hidden;">
    <div class="title_area">
        <div class="title_text">
            ...
            <a href="javascript:void(0);" class="in_house_work_submenuitem active" url="<?php echo $html->url(array('controller' => 'encounters', 'action' => 'in_house_work_injections', 'encounter_id' => $encounter_id)); ?>" style="float: none;">Injections</a>
    ...
    <div id="lab_records_area" class="tab_area"> 
        <div> 
            ...
            $injection_name = $AdministrationPointOfCare['AdministrationPointOfCare']['injection_name'];
            ?>
            <div class="poc-wrapper" ><label class="label_check_box_home" id="label_injection_<?php echo $point_of_care_id; ?>" name="label_injection_<?php echo $point_of_care_id; ?>" ><input name="injection_<?php echo $point_of_care_id; ?>" id="injection_<?php echo $point_of_care_id; ?>" type="checkbox" class="fast-chkbox" value="<?php echo $point_of_care_id; ?>" <?php echo (in_array(strtolower($injection_name), $EncounterPointOfCare))?'checked':''; ?> data-poc_id="<?php echo $point_of_care_id; ?>" data-poc_injection_name="<?php echo $injection_name; ?>" /> &nbsp;<?php echo $injection_name; ?></label><button class="poc-button" id="rerun_<?php echo $point_of_care_id; ?>" >&nbsp;</button></div>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="clear"></div>
    <div id="lab_records_area" class="tab_area"> 
        <div id="in_house_work_injection_form_area" style="padding-top:15px;"></div>
    </div>
</div>
<style>
