...
$mainURL = $html->url(array('action' => 'in_house_work_radiology', 'encounter_id' => $encounter_id)) . '/';
...
<script language="javascript" type="text/javascript">
    function addData(point_of_care_id, radiology_procedure_name)
    {
        
        var val = (jQuery('#radiology_procedure_'+point_of_care_id).is(':checked'))?'1':'0';

        if(val==1)
        {           
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_radiology/encounter_id:<?php echo $encounter_id; ?>/task:addRadiologyTest/', 
            {
              'data[administration_point_of_care_id]': point_of_care_id,
              'data[item_value]': radiology_procedure_name
            }, 
            function(data)
            {
            }
            );
            $('#label_radiology_procedure_'+point_of_care_id).css('cursor', 'pointer');
        }
        else
        {
            $('#in_house_work_radiology_form_area').hide();
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_radiology/encounter_id:<?php echo $encounter_id; ?>/task:deleteRadiologyTest/', 
            {
              'data[item_value]' : radiology_procedure_name
            }, 
            function(data)
            ...
            $('#label_radiology_procedure_'+point_of_care_id).css('cursor', 'auto');
        }
    }
    
    function loadData(point_of_care_id, radiology_procedure_name)
    {
        var val = (jQuery('#radiology_procedure_'+point_of_care_id).is(':checked'))?'1':'0';
        if(val==1)
        {
        $('#in_house_work_radiology_form_area').show();
        $('#in_house_work_radiology_form_area').html('');
        $("#imgLoad").show();
            $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_radiology/encounter_id:<?php echo $encounter_id; ?>/task:checkRadiologyTest/', 
                {
                   'data[item_value]' : radiology_procedure_name
                }, 
            function(data)
            {
               ...
               if(data.radiology_test['exist']=='yes')
               {
                  $.post('<?php echo $this->Session->webroot; ?>encounters/in_house_work_radiology_data/encounter_id:<?php echo $encounter_id; ?>/',     {'radiology_procedure_name':radiology_procedure_name}, 
                  function(data){
                    $('#in_house_work_radiology_form_area').html(data);
                    $("#imgLoad").hide();
	...
	function loadInitialData()
	{
		...
		$.post('<?php echo $this->Session->webroot; ?>encounters/<?php echo $this->params['action']; ?>_data/encounter_id:<?php echo $encounter_id; ?>/', 'radiology_procedure_name='+'<?php echo $init_point_of_care_name; ?>', 
		function(data)
		{ 
		   $('#in_house_work_radiology_form_area').html(data);
		   $("#imgLoad").hide();
    ...
    {   
        ...
        $.post(
            '<?php echo $this->Session->webroot; ?>encounters/in_house_work_radiology/encounter_id:<?php echo $encounter_id; ?>/task:get_admin_poc_radiology/', 
            '', 
            function(data)
            {
            //alert(data.lab_items.length);
            if(data.radiology_items.length > 0)
                {            
                    for(var i = 0; i < data.radiology_items.length; i++)
                    {
                    var point_of_care_id = data.radiology_items[i].AdministrationPointOfCare.point_of_care_id;
        ...
        $('#previousRecordsbtn').click(function()
        {
            ...
            loadTab($(this), "<?php echo $html->url(array('controller' => 'encounters', 'action' => 'poc_previous_records', 'encounter_id' => $encounter_id)); ?>");
				...
				$('.fast-chkbox').each(function(){
					...
					var radiology_name = $(this).data('poc_radiology_name');
					$(this).click(function(){
						addData(poc_id, radiology_name);
				...
				$('.poc-button').each(function(){
					...
					var radiology_name = $(this).closest('.poc-wrapper').find('.fast-chkbox').data('poc_radiology_name');
					
					
					$(this).click(function(){
						loadData(poc_id, radiology_name);
					});
				});        
...
<div id="poc_area" style="overflow: hidden;">
    <div class="title_area">
        <div class="title_text">
            ...
            <a href="javascript:void(0);" class="in_house_work_submenuitem active" url="<?php echo $html->url(array('controller' => 'encounters', 'action' => 'in_house_work_radiology', 'encounter_id' => $encounter_id)); ?>" style="float: none;">Radiology</a>
    ...
    <div id="lab_records_area" class="tab_area"> 
        <div> 
            ...
            $radiology_procedure_name = $AdministrationPointOfCare['AdministrationPointOfCare']['radiology_procedure_name'];
            ?>
            <div class="poc-wrapper" ><label class="label_check_box_home" id="label_radiology_procedure_<?php echo $point_of_care_id; ?>" name="label_radiology_procedure_<?php echo $point_of_care_id; ?>" ><input name="radiology_procedure_<?php echo $point_of_care_id; ?>" id="radiology_procedure_<?php echo $point_of_care_id; ?>" type="checkbox" class="fast-chkbox" value="<?php echo $point_of_care_id; ?>" <?php echo (in_array(strtolower($radiology_procedure_name), $EncounterPointOfCare))?'checked':''; ?> data-poc_id="<?php echo $point_of_care_id; ?>" data-poc_radiology_name="<?php echo $radiology_procedure_name; ?>" /> &nbsp;<?php echo $radiology_procedure_name; ?></label><button class="poc-button" id="rerun_<?php echo $point_of_care_id; ?>" >&nbsp;</button></div>
    ...
    <div class="clear"></div>
    <div id="lab_records_area" class="tab_area">
        <div id="in_house_work_radiology_form_area" style="padding-top:15px;"></div>
    </div>
</div>
<style>
